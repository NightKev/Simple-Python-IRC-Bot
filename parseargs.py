...
def getargs():
    ...
    botargs.add_argument('-n','--nick', default="SpireBot", help="The nickname of the bot.")
    botargs.add_argument('-p','--password', default='', help="The bot's nickserv pass (only useful if the IRC server has nickname registration services and you have registered the nickname used by the bot).")
    botargs.add_argument('--ident', default='SpireBot', help="The 'ident' of the bot. Used in the second part of the 'user!<ident>@host' scheme. Defaults to 'SpireBot'.")
    botargs.add_argument('--real-name', default='SpireBot', help="Sets the bot's 'real name'.")
    botargs.add_argument('-t','--trigger', default='~', help="The trigger to use for bot commands (ex: '!google abc' or '.weather 90210'). Defaults to '~' if not set.")
    botargs.add_argument('--modes', default='+Bix', help="Initial usermodes to set on the bot upon joining the server. Defaults to '+Bix'.")
    ...
    miscargs.add_argument('--identd-port', default='113', help="Set a port other than 113 to be used locally by the identd server before being forwarded to 113 by a router.")
